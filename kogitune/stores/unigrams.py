...
def extract_vocab_score_from_spiece(model_path):
    ...
    vocab = [
        (sp_model.id_to_piece(id), sp_model.get_score(id))
...
def save_tokenizer(tokenizer, save_path=None, text="🦊 token化、大丈夫？"):
    # FastTokenizerの作成
...
def train_spm(input_files, aargs):
    kwargs = adhoc.extract_kwargs(
        ...
        aargs,
        excludes=["save_path", "files", "use_wakachi", "test_sentence"],
    )
